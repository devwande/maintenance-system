...
const AdminDashboard = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (adminDormitory && adminDormitory !== "General Admin") {
        filteredData = response.data.filter((request: MaintenanceRequest) =>
          request.location.includes(adminDormitory)
        );
      }
  ...
  const handleAssignCategory = async (newCategory: string) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:3001/api/admin/assign/${selectedRequest._id}`,
        {
  ...
  const handleUpdatePriority = async (newPriority: string) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:3001/api/admin/priority/${selectedRequest._id}`,
        {
  ...
  const exportByHall = (hall: string): void => {
    const hallRequests = filteredRequests.filter((request) =>
      request.location.includes(hall)
    );
  ...
  const exportByMonth = (month: string): void => {
    const monthRequests = filteredRequests.filter((request) => {
      const requestMonth = new Date(request.createdAt)
        .toISOString()
        .slice(0, 7);
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart
                  data={Object.entries(stats.categories).map(
                    ([category, count]) => ({
                      category,
                      count,
                    })
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {workerStats.map((worker) => (
                    <tr key={worker._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {worker.averageResolutionTime
                            ? `${worker.averageResolutionTime.toFixed(1)} hrs`
                            : "N/A"}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {worker.workload.inProgress} in progress
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
              <div className="flex-1 flex justify-between sm:hidden">
                ...
                <button
                  ...
                  disabled={
                    page >= Math.ceil(filteredRequests.length / rowsPerPage)
                  }
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    {selectedRequest.studentRegNumber}
                  </p>
                </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {priorities.map((priority) => (
                      <button
                        ...
                        disabled={
                          isAssigning || selectedRequest.priority === priority
                        }
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {categories.map((category) => (
                      <button
                        ...
                        disabled={
                          isAssigning || selectedRequest.category === category
                        }
                        className={`px-3 py-1.5 rounded-md ${
